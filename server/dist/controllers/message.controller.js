...
const verifyMembership = async (userId, chatId, chatType) => {
    if (chatType === "direct") {
...
export const sendMessage = async (req, res) => {
    try {
        ...
        let uplaodedAttachments = [];
        if (attachments && attachments.length > 0) {
            const upload = await Promise.all(attachments.map(async (attachment) => {
        ...
        const chatRelation = chatType === "direct" ? { directChatId: chatId } :
